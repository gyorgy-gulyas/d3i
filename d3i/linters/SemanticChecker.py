...
class SemanticChecker(d3i.elements.ElementVisitor):
    ...
    def visitEvent(self, event: event, parentData: Any) -> Any:
        ...
        for inherit in event.inherits:
            ...
            if (base_class == None):
                self.__error__(event, f"The element '{inherit.getText()}' referred in inheritance is not found. {message}")
            elif (isinstance(base_class, event) == False):
                self.__error__(event, f"The element '{inherit.getText()}' referred in inheritance is not an event.")
    ...
    def visitValueObject(self, value_object: value_object, parentData: Any) -> Any:
        ...
        for inherit in value_object.inherits:
            ...
            if (base_class == None):
                self.__error__(value_object, f"The element '{inherit.getText()}' referred in inheritance is not found. {message}")
            elif (isinstance(base_class, value_object) == False):
                self.__error__(value_object, f"The element '{inherit.getText()}' referred in inheritance is not a value object.")
    ...
    def visitEnity(self, entity: entity, parentData: Any) -> Any:
        ...
        for inherit in entity.inherits:
            ...
            if (base_class == None):
                self.__error__(entity, f"The element '{inherit.getText()}' referred in inheritance is not found. {message}")
            elif (isinstance(base_class, entity) == False):
                self.__error__(entity, f"The element '{inherit.getText()}' referred in inheritance is not an entity.")
