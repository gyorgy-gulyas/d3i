...
class DotnetEmitter:
    ...
    def nullableDirective(self) -> str:
        """
        Every generated file says what it means by a reference type.

        It has to be IN the file. The compiler holds auto-generated files - the ones carrying the
        '<auto-generated>' header above - outside the project's nullable context whatever the csproj
        says, and only an explicit directive brings them back. So this is not a workaround for a
        project setting; it is the only place the setting can be made for generated code.

        It is the full 'enable' and not 'enable annotations': the point is that '@optional' becomes
        something the compiler enforces rather than something the model merely says. That costs the
        emitter three things - a non-null starting value for every required reference field, an
        'object?' on the Equals override, and a real required-check in the validator - and buys a
        consumer who no longer null-checks what the model calls mandatory.
        """
    ...
    def beginFile(self, output_path: str, element: base_element, subDirectoryName: str, prefix: str = "", postfix: str = "", current_namespace:str=None, name_override: str = None) -> dotnet_code:
        ...
        buffer.write("\n")
        buffer.write(self.nullableDirective())
    ...
    def dataClassEqualsAndHashCodeText(self, element: internal_scoped_base_element, inherits: List[qualified_name], name: str, members: List[hinted_base_element], code: dotnet_code, indent: int = 1) -> str:
        ...
        # '{name}?', because that is what IEquatable<T>.Equals declares - the null check on the next
        # line is the whole reason the interface allows it. A narrower parameter is CS8767.
        ...
        # 'object?', because that is what object.Equals declares. Under '#nullable enable' a
        # narrower override is CS8765 - the base promises to accept null and this one would not.
    ...
    def dataClassCloneText(self, element: internal_scoped_base_element, inherits: List[qualified_name], name: str, members: List[hinted_base_element], code: dotnet_code, indent: int = 1) -> str:
        ...
        for base in bases:
            ...
            for member in base.members:
                if( member.find_decorator("system_field") != None ):
                    continue
                buffer.write(self.dataClassMemberCloneText(member.name, member.type, code, dst="clone.", src="", indent=indent+1, is_optional=member.find_decorator("optional") != None))
                pass
            buffer.write(f"{utils.tab(indent+1)}// end: {base.name}\n\n")
        # Write each own member
        for member in element.members:
    ...
    def dataClassMemberEqualsText(self, memberName: str, memberType: type, code: dotnet_code, dst: str, src: str, indent: int) -> str:
        match memberType.kind:
            ...
            case type.Kind.Ref:
                # A 'ref X' is an EntityId<X>: a readonly record struct, so it compares like any
                # other value. It used to take the reference branch below and be compared against
                # null - which only compiled because EntityId has an implicit conversion FROM
                # string, so the null was quietly becoming a string. The Clone path had already
                # been fixed for the same reason; these two had not.
                return self.dataClassMemberEqualsText_Primitive(memberName, memberType, code, dst, src, indent)
    ...
    def dataClassMemberHashText(self, memberName: str, memberType: type, code: dotnet_code, dst: str, src: str, indent: int) -> str:
        match memberType.kind:
            ...
            case type.Kind.Ref:
                # a struct - see the note on the Equals dispatcher above
                return self.dataClassMemberHashText_Primitive(memberName, memberType, code, dst, src, indent)
            case type.Kind.Reference:
    ...
    def dataClassMemberCloneText(self, memberName: str, memberType: type, code: dotnet_code, dst: str, src: str, indent: int, is_optional: bool = False) -> str:
        # `is_optional` decides one thing: whether the clone expression is allowed to hand back null.
        # A required field is declared non-nullable, so the clone carries that declaration forward
        # with a '!' rather than widening the field - the promise lives in one place (the
        # declaration) and is checked in one place (the validator), not restated at every use.
        match memberType.kind:
            case type.Kind.Primitive:
                return self.dataClassMemberCloneText_Primitive(memberName, memberType, code, dst, src, indent, is_optional)
            case type.Kind.Ref:
                return self.dataClassMemberCloneText_Ref(memberName, memberType, code, dst, src, indent)
            case type.Kind.Reference:
                return self.dataClassMemberCloneText_Reference(memberName, memberType, code, dst, src, indent, is_optional)
            case type.Kind.List:
    ...
    def dataClassMemberCloneText_Primitive(self, memberName: str, memberType: type, code: dotnet_code, dst: str, src: str, indent: int, is_optional: bool = False) -> str:
        if (is_optional == False and memberType.primtiveKind == primitive_type.PrimtiveKind.Bytes):
    ...
    def dataClassMemberCloneExpression(self, memberName: str, memberType: type, code:dotnet_code ) -> str:
        ...
        match memberType.primtiveKind:
            ...
            case primitive_type.PrimtiveKind.String:
                # A string is immutable, so the assignment IS the copy. Building a new one from the
                # characters bought nothing and cost an allocation per field per clone - and it
                # dereferenced a field that may legitimately be null when it is @optional.
                buffer.write( f"{memberName}")
            ...
            case primitive_type.PrimtiveKind.Bytes:
                # An array IS mutable, so this copy is real - but the source may be null when the
                # field is @optional, hence the null-conditional.
    ...
    def dataClassMemberCloneText_Reference(self, memberName: str, memberType: type, code: dotnet_code, dst: str, src: str, indent: int, is_optional: bool = False) -> str:
        referenced_element: base_element = Engine.get_referenced_element(memberType.parent, memberType.reference_name)
        ...
        if (isinstance(referenced_element, enum) == True):
            ...
            buffer.write(f"{utils.tab(indent)}{dst}{memberName} = ")
            suppress = "" if is_optional == True else "!"
            buffer.write(f"{src}{memberName}?.Clone(){suppress};\n")
    ...
    def compositeText(self, composite: composite, code: dotnet_code, indent: int = 1) -> dotnet_code:
        ...
        for member in composite.members:
            buffer.write(self.documentLines(member, indent+1))
            # Write each member
            buffer.write(self.propertyText(member, code, indent+1, with_initializer=False))
        buffer.write(f"{utils.tab(indent)}}}\n")
    ...
    def interfaceRestInternalClientText(self, interface: interface, code: dotnet_code, indent: int = 1) -> dotnet_code:
        ...
        for operation in interface.operations:
            ...
            if( operation.operation_return != None ):
                ...
                # A success status with a body that deserializes to null is not a success. It used
                # to go straight into Success(null) and surface as a null reference at the caller,
                # one stack frame too late to say what happened.
                buffer.write(f"{utils.tab(indent+4)}if (value is null)\n")
                buffer.write(f"{utils.tab(indent+5)}return Response<{self.typeText( operation.operation_return.type, code, fullName=True)}>.Failure( Statuses.InternalError, \"The server answered '{operation.name}' with a success status and an empty body.\" );\n\n")
    ...
    def interfaceRestPublicClientText(self, interface: interface, code: dotnet_code, indent: int = 1) -> dotnet_code:
        ...
        for operation in interface.operations:
            ...
            if( operation.operation_return != None ):
                ...
                buffer.write(f"{utils.tab(indent+6)}if (value is null)\n")
                buffer.write(f"{utils.tab(indent+7)}return Response<{self.typeText( operation.operation_return.type, code, fullName=True)}>.Failure( Statuses.InternalError, \"The server answered '{operation.name}' with a success status and an empty body.\" );\n\n")
    ...
    def propertyText(self, member: hinted_base_element, code: dotnet_code, indent: int, with_initializer: bool = True) -> str:
        """
        One property. `with_initializer` is False where an initializer is not legal C# - a composite
        becomes an INTERFACE, and an interface property cannot have one. The class that inherits the
        composite gets the same member emitted again, with the initializer, so nothing is lost.
        """
        buffer = io.StringIO()
        ...
        is_optional: bool = member.find_decorator("optional") != None
        if (is_optional == True):   # @optional -> nullable; unmarked = required
            type_text = type_text + "?"
        buffer.write(f"{utils.tab(indent)}public {type_text} {member.name} {{ get; set; }}")
        if (with_initializer == False):
            pass
        elif(member.type.kind == type.Kind.List or member.type.kind == type.Kind.Map ):
            buffer.write(f" = new();")
        elif (is_optional == False and self.__isReferenceType(member.type) == True):
            # A required reference field. Under '#nullable enable' the compiler wants to know it is
            # not null, and there is no honest value to give it here:
            #
            #   '= new()' on a nested type would be worse than nothing - it manufactures an empty
            #   object that then breaks its OWN validation rules, which is exactly the bug the
            #   sample already found once (a customer that could not be registered because the
            #   service invented an empty address).
            #
            #   'string.Empty' would make "not set" and "set to nothing" the same value, and then
            #   the required-check below could never tell them apart.
            #
            # So the field starts as null and SAYS it does not, and the promise is kept where a
            # promise about data is kept anyway: ValidateInto refuses the object before the store
            # writes it. Which is more than happened before - until now nothing checked at all.
            buffer.write(f" = null!;")
        buffer.write(f"\n")

        return buffer.getvalue()

    def __isReferenceType(self, the_type: type) -> bool:
        """
        Is this a .NET reference type - the only kind '#nullable enable' has an opinion about?

        An enum arrives here as a reference_type (it is referenced BY NAME in the model) and is a
        value type in C#, so the question cannot be answered from the model's own type kind alone.
        """
        match the_type.kind:
            case type.Kind.Primitive:
                return the_type.primtiveKind in (
            ...
            case type.Kind.Reference:
                referenced = Engine.get_referenced_element(the_type.parent, the_type.reference_name)
                # unresolved is the linter's business; assume the common case rather than crash
                if (referenced == None):
                    return True
                return isinstance(referenced, enum) == False
            case type.Kind.List | type.Kind.Map:
                return True

        # Ref is an EntityId<T>, which is a readonly record struct
        return False
