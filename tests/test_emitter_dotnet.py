...
domain WebShop {
    context Sales {
        ...
        aggregate Order {
            root entity OrderHeader inherits Base {
                @partitionKey
                orderId:string
...
"""
    ...
    def test_a_generated_file_says_what_it_means_by_a_reference_type(self):
        # It has to be IN the file: the compiler holds '<auto-generated>' files outside the
        # project's nullable context whatever the csproj says, so a project setting cannot reach
        # them and an explicit directive is the only way in.
        emitted = self.__emitOne(self.NULLABLE_SOURCE, "OrderHeader.cs")
        self.assertIn("#nullable enable", emitted.content)

    def test_a_required_reference_field_starts_non_null_and_an_optional_one_does_not(self):
        emitted = self.__emitOne(self.NULLABLE_SOURCE, "OrderHeader.cs")

        # required: declared non-nullable, and told so - the runtime truth is the validator's job
        self.assertIn("public string orderId { get; set; } = null!;", emitted.content)
        self.assertIn("public WebShop.Sales.Address billingAddress { get; set; } = null!;", emitted.content)

        # optional: nullable, and nothing to initialise
        self.assertIn("public string? note { get; set; }\n", emitted.content)
        self.assertIn("public WebShop.Sales.Address? shippingAddress { get; set; }\n", emitted.content)

        # a value type has no null to worry about: an enum arrives as a reference_type in the model
        # and is a struct in C#, and a 'ref' is an EntityId, which is one too
        self.assertIn("public WebShop.Sales.Statuses status { get; set; }\n", emitted.content)
        self.assertIn("public EntityId<OrderHeader> customer { get; set; }\n", emitted.content)

        # a list already had one, and it is a better one than null
        self.assertIn("public List<string> lines { get; set; } = new();", emitted.content)

    def test_a_composite_becomes_an_interface_and_an_interface_property_has_no_initializer(self):
        # '= null!' on an interface property is CS8053. The class that inherits the composite emits
        # the same member again, with the initializer, so nothing is lost.
        emitted = self.__emitOne(self.NULLABLE_SOURCE, "IBase.cs")
        self.assertIn("public partial interface IBase", emitted.content)
        self.assertIn("public string humanKey { get; set; }\n", emitted.content)
        self.assertNotIn("null!", emitted.content)

    def test_equals_accepts_null_because_the_contract_it_implements_does(self):
        emitted = self.__emitOne(self.NULLABLE_SOURCE, "OrderHeader.cs")
        self.assertIn("public bool Equals( OrderHeader? other )", emitted.content)
        self.assertIn("public override bool Equals(object? obj)", emitted.content)

    def test_a_ref_is_compared_as_the_value_it_is(self):
        # An EntityId<T> is a readonly record struct. Comparing it to null only ever compiled
        # because EntityId converts implicitly FROM string, so the null was quietly becoming one.
        emitted = self.__emitOne(self.NULLABLE_SOURCE, "OrderHeader.cs")
        self.assertIn("if(customer != other.customer) return false;", emitted.content)
        self.assertNotIn("customer == null", emitted.content)
        self.assertIn("hash.Add(customer);", emitted.content)

    def test_cloning_a_string_does_not_copy_it(self):
        # A string is immutable, so the assignment IS the copy. The old 'new string(x.ToCharArray())'
        # bought nothing, cost an allocation per field per clone, and dereferenced a field that is
        # allowed to be null when it is @optional.
        emitted = self.__emitOne(self.NULLABLE_SOURCE, "OrderHeader.cs")
        self.assertNotIn("ToCharArray", emitted.content)
        self.assertIn("clone.orderId = orderId;", emitted.content)
